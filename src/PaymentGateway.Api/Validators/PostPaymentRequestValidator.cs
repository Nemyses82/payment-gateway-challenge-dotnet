...
public class PostPaymentRequestValidator : AbstractValidator<PostPaymentRequest>
{
    public PostPaymentRequestValidator()
    {
        ...
        RuleFor(x => x.Currency)
            ...
            .Must(x => new List<string> { "GBP", "EUR", "CHF" }.Contains(x)).WithMessage("Currency must contain only GBP, EUR or CHF.");
    ...
    {
        return request.ExpiryYear >= 0 && request.ExpiryYear <= DateTime.Now.Year && request.ExpiryMonth >= 1 && request.ExpiryMonth <= 12;
    }
